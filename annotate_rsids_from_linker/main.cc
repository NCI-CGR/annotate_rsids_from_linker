/*!
  \file annotate_rsids_from_linker.cc
  \brief approximately convert chr:pos ids to rsids
...
void splitline(const std::string &line, std::vector<std::string> *vec,
               const std::string &sep) {
  if (!vec) throw std::domain_error("splitline: called with null pointer");
  vec->clear();
...
class rescue_annotation {
 public:
  rescue_annotation() : _input_index(0), _ref(""), _alt(""), _updated_id("") {}
...
bool rescue_annotation::alleles_consistent(const std::string &ref,
  ...
  if (test_against_ref || test_against_alt) {
    for (std::vector<std::string>::const_iterator iter = split_alt.begin();
         iter != split_alt.end(); ++iter) {
      if (!iter->compare(
              test_against_ref
                  ? get_ref()
                  : (test_against_alt ? get_alt() : std::string("")))) {
        return true;
      }
    }
  }
...
void stream_update(const std::string &analysis_filename,
  ...
  try {
    ...
    while (input_analysis->getline(&analysis_line)) {
      ...
      while (true) {
        ...
        if (!(strm2 >> chrpos_id >> rsid))
          throw std::domain_error("unable to parse linker file \"" +
        ...
        if (!chrpos_id.compare(analysis_chrpos)) {
          ++total_updated;
          // note: these two rsIDs are annotation failures in dbSNP
          // due to weird cross-chromosome homology in pseudogenes.
          // as these are the only variants I've found so far affected by
          // this issue, I'm just putting in a hard-coded fix. if I find
          // more, I'll improve the generalisability of the fix
          if (rsid.compare(".") && rsid.compare("rs1192281978") &&
              rsid.compare("rs1160972848")) {
            ++valid_updated;
            rsid += ":" + refalt;
          } else {
            rsid = analysis_chrpos;
          }
          if (output) {
            std::ostringstream o;
          ...
          linker_line = "";
          break;
        } else if ((linker_chr == analysis_chr && linker_pos > analysis_pos) ||
                   linker_chr > analysis_chr) {
          // something terrible has happened with the linker, flush the
          // analysis result without an ID
          ++not_present_in_linker;
          if (output) {
            std::ostringstream o;
            o << analysis_chr << '\t' << analysis_pos << "\t"
              << analysis_chrpos;
    ...
    if (output) {
      output->close();
      delete output;
      std::cout << output_filename << ": updated " << total_updated
                << ", of which " << valid_updated << " had nonmissing rsids; "
                << not_present_in_linker
                << " were either absent from the linker or unsorted and have "
                   "been set to \".\""
                << std::endl;
    } else {
      std::cout << "from first pass, updated " << valid_updated
                << ", didn't manage to update "
                << (total_updated - valid_updated + not_present_in_linker)
                << std::endl;
    }
  } catch (...) {
...
void dbsnp_rescue(const std::string &filename,
  ...
  try {
    ...
    while (input->getline(&line) && n_updated != target->size()) {
      ...
      splitline(line, &vec, "\t");
      if (vec.size() < 5)
        throw std::domain_error("inadequate entries on dbsnp line \"" + line +
                                "\"");
      if (vec.at(4).empty()) vec.at(4) = ".";
      if ((finder = target->find(vec.at(0) + ":" + vec.at(1))) !=
          target->end()) {
    ...
    delete input;
    input = 0;
    std::cout << "of " << target->size() << " possible updates, updated "
              << n_updated << " ("
              << (static_cast<double>(n_updated) / target->size()) << ")"
              << std::endl;
  } catch (...) {
...
void update_from_memory(
    const std::string &input_filename, std::vector<std::string> *updated_ids,
  ...
  try {
    ...
    for (std::map<std::string, rescue_annotation>::const_iterator iter =
      ...
      if (!iter->second.get_updated_id().empty()) {
        updated_ids->at(iter->second.get_index()) =
            iter->second.get_updated_id();
      }
    }
    input->getline(&line);
...
int main(int argc, char **argv) {
  if (argc < 4)
    throw std::domain_error("usage: \"" + std::string(argv[0]) +
                            " input_analysis_filename linker_filename "
                            "[dbsnp_vcf_filename] output_filename\"");
  ...
  if (dbsnp_filename.empty()) {
    std::cout << "running one-shot against prepared linker file "
              << linker_filename << std::endl;
  } else {
    std::cout << "running multipass run against dbSNP " << dbsnp_filename
              << " with a preprocessing step against the linker "
              << linker_filename << std::endl;
  }
  std::cout << "running stream_update against linker" << std::endl;
  stream_update(analysis_filename, linker_filename,
                dbsnp_filename.empty() ? output_filename : std::string(""),
                dbsnp_filename.empty() ? NULL : &updated_ids,
                dbsnp_filename.empty() ? NULL : &rescue_chrpos);
  if (!rescue_chrpos.empty()) {
    std::cout << "running rescue pass against dbSNP for "
              << rescue_chrpos.size() << " variants" << std::endl;
    dbsnp_rescue(dbsnp_filename, &rescue_chrpos);
  }
  if (!updated_ids.empty()) {
    std::cout << "emitting consensus results from both passes to "
              << output_filename << std::endl;
    update_from_memory(analysis_filename, &updated_ids, rescue_chrpos,
                       output_filename);
  }
  return 0;
}
