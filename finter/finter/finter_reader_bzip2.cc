...
void annotate_rsids_from_linker::finter_reader_bzip2::open(const char *filename) {
  ...
  if (!_raw_input)
    throw std::domain_error(
        "annotate_rsids_from_linker::finter_reader_bzip2: cannot open file \"" +
        std::string(filename) + "\"");
  int error = 0;
  _bz_input = BZ2_bzReadOpen(&error, _raw_input, 0, 0, NULL, 0);
  if (error == BZ_CONFIG_ERROR) {
    throw std::domain_error(
        "annotate_rsids_from_linker::finter_reader_bzip2::open: bzip2 reading "
...
void annotate_rsids_from_linker::finter_reader_bzip2::close() {
  if (_bz_input) {
    int error = 0;
...
void annotate_rsids_from_linker::finter_reader_bzip2::clear() {
  _good = true;
  _bad = _fail = false;
  _buf_remaining = _buf_read = 0;
}

bool annotate_rsids_from_linker::finter_reader_bzip2::is_open() const {
  return (_raw_input && _bz_input);
}

char annotate_rsids_from_linker::finter_reader_bzip2::get() {
  if (!_buf_remaining) {
    refresh_buffer();
  }
  if (!_buf_remaining)
    throw std::domain_error(
        "annotate_rsids_from_linker::finter_reader_bzip2::get: end of file");
  --_buf_remaining;
...
void annotate_rsids_from_linker::finter_reader_bzip2::refresh_buffer() {
  if (!eof()) {
    int error = 0;
    int num_read =
    ...
    if (error == BZ_PARAM_ERROR || error == BZ_IO_ERROR ||
      ...
      throw std::domain_error(
          "annotate_rsids_from_linker::finter_reader_bzip2::refresh_buffer: "
          "bzip2 reports "
          "read called on stream opened as write");
    }
