...
void annotate_rsids_from_linker::finter_writer_bzip2::open(
  ...
  if (!_raw_output)
    throw std::domain_error(
        "annotate_rsids_from_linker::finter_writer_bzip2: cannot open file \"" +
        std::string(filename) + "\"");
  int error = 0;
  _bz_output = BZ2_bzWriteOpen(&error, _raw_output, 9, 0, 0);
  if (error == BZ_CONFIG_ERROR) {
    throw std::domain_error(
        "annotate_rsids_from_linker::finter_writer_bzip2::open: bzip2 writing "
...
void annotate_rsids_from_linker::finter_writer_bzip2::close() {
  int error = 0;
  if (_bz_output) {
...
void annotate_rsids_from_linker::finter_writer_bzip2::clear() {
  _good = true;
  _bad = _fail = false;
}

bool annotate_rsids_from_linker::finter_writer_bzip2::is_open() const {
  return (_raw_output && _bz_output);
}

void annotate_rsids_from_linker::finter_writer_bzip2::put(char c) {
  int error = 0;
  ...
  if (error == BZ_PARAM_ERROR || error == BZ_IO_ERROR) {
    ...
    throw std::domain_error(
        "annotate_rsids_from_linker::finter_writer_bzip2::put: bzip reports "
        "write operation called on read handle");
  }
}

void annotate_rsids_from_linker::finter_writer_bzip2::writeline(
  ...
  if (error == BZ_PARAM_ERROR || error == BZ_IO_ERROR) {
    ...
    throw std::domain_error(
        "annotate_rsids_from_linker::finter_writer_bzip2::writeline: bzip "
        "reports "
        "write operation called on read handle");
  }
}
...
void annotate_rsids_from_linker::finter_writer_bzip2::write(char *buf,
  ...
  if (error == BZ_PARAM_ERROR || error == BZ_IO_ERROR) {
    ...
    throw std::domain_error(
        "annotate_rsids_from_linker::finter_writer_bzip2::write: bzip reports "
        "write operation called on read handle");
  }
}
